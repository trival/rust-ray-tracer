...
impl Camera {
    fn new(origin: DVec3, direction: DVec3, near: f64) -> Self {
        Self {
            origin,
            direction,
            near,
    ...
    fn view_port_directions(&self) -> (DVec3, DVec3) {
        let u = self.direction.cross(self.up).normalize();
    ...
    fn render(
        ...
        let viewport_center = self.origin + self.direction * self.near;
...
impl Scene {
    fn closest_object(&self, ray: &Ray) -> Option<(&SceneObject, f64)> {
        ...
        closest_object.map(|object| (object, closest_t))
    }
}
...
fn ray_color(ray: &Ray, scene: &Scene, depth: usize) -> DVec3 {
    ...
    if let Some((obj, t)) = scene.closest_object(ray) {
        ...
        let reflect_ray = reflect(ray.dir, hit_normal);

        let mut scatter_dir = reflect_ray + random_in_unit_sphere().normalize() * 0.6;
        if scatter_dir.length_squared() < 0.001 {
            scatter_dir = reflect_ray;
        }
        ...
        DVec3::ONE.lerp(dvec3(0.5, 0.7, 1.), t)
    }
}
